...
d <- fread('../src/data/W241 Survey StoryTelling Experiment_April 17, 2022_16.35.csv')
...
quiz_answer_key_social <- tibble(question = 
                          ...
                          answer = 
                            c("FamilyOK", 
                              ...
                              "six", "Because of teens’ impulsive natures"))
...
d_2 <- d %>% 
  ...
  mutate(sc1 = if_else(`treatment_question` == "alien_treatment_bool", `sc1`, 0),
         sc3 = if_else(`treatment_question` == "recyc_treatment_bool", `sc3`, 0),
         sc2 = if_else(`treatment_question` == "social_treatment_bool", `sc2`, 0),
...
ggplot(dat %>% rename(`Treatment Group` = treatment_bool_factor),
       aes(x=quiz_perc_correct,
           y = (..count..)/sum(..count..),
           fill=`Treatment Group`)) + 
  geom_histogram(alpha=0.2, position="identity") + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1L)) +
  xlab("Percentage of Correct Answers on the Quiz") +
...
# Treatment by topic
model_basic_topic <- dat %>% 
  lm(quiz_perc_correct ~ treatment_bool + question_topic, data = .)

summary(model_basic_topic)


# Treatment by topic 2
model_basic_topic_2 <- dat %>% 
  lm(quiz_perc_correct ~ treatment_bool*question_topic, data = .)

summary(model_basic_topic_2)




# For and Against Avgs
quiz_for_correct <- d_final_pre_regressions %>% 
  select(response_id, quiz_for_correct) %>% 
  unique() %>% 
  select(quiz_for_correct)


quiz_against_correct <- d_final_pre_regressions %>% 
  select(response_id, quiz_against_correct) %>% 
  unique() %>% 
  select(quiz_against_correct)

t.test(quiz_for_correct, quiz_against_correct)


# See if there is a difference on the 'for' questions
model_for_basic <- d_final_pre_regressions %>% 
...
model_against_basic <- d_final_pre_regressions %>% 
...
model_basic_recy <- dat %>% 
  select(response_id, recycle_question, treatment_bool, quiz_perc_correct) %>% 
...
# Clustered Standard Errors and Stargazer Chart
model_basic$clustered_se <- sqrt(diag(vcovCL(model_basic, 
                                             cluster = ~response_id)))

model_basic_topic$clustered_se <- sqrt(diag(vcovCL(model_basic_topic, 
                                             cluster = ~response_id)))

model_basic_topic_2$clustered_se <- sqrt(diag(vcovCL(model_basic_topic_2, 
                                             cluster = ~response_id)))

model_basic_alien$clustered_se <- sqrt(diag(vcovCL(model_basic_alien, 
                                             cluster = ~response_id)))

model_basic_recy$clustered_se <- sqrt(diag(vcovCL(model_basic_recy, 
                                             cluster = ~response_id)))

model_basic_social$clustered_se <- sqrt(diag(vcovCL(model_basic_social, 
                                             cluster = ~response_id)))

stargazer(model_basic,
          model_basic_topic,
          model_basic_topic_2,
          # model_basic_alien, 
          # model_basic_recy,
          # model_basic_social,
          dep.var.labels=c("Model Type"),
          # column.labels=c("Basic","By Topic","alien","recycle","social media"),
          # column.labels=c("all","all","alien","recycle","social media"),
          title="Regression Results with Clustered SE on `Response ID`",
          type="text", 
          se = list(model_basic$clustered_se,
                    model_basic_topic$clustered_se,
                    model_basic_topic_2$clustered_se
                    
                    # model_basic_alien$clustered_se,
                    # model_basic_recy$clustered_se,
                    # model_basic_social$clustered_se
                    ),
...
model_basic_cat_or_num <- dat_cat_or_num %>% 
  # filter(question_topic %like% 'soc') %>% 
...
model_cat_or_num_with_treatment <- dat_cat_or_num_2 %>% 
  lm(cat_or_num_question_correct ~ treatment_bool + cat_or_num_question_label + treatment_bool*cat_or_num_question_label,
     data = .)

summary(model_cat_or_num_with_treatment)
# It does not appear that the treatment impacted whether people did better or worse on the number vs categorical questions


# Part 3 - Question Type and Question Topic
model_cat_or_num_with_question_type <- dat_cat_or_num_2 %>% 
     lm(cat_or_num_question_correct ~ cat_or_num_question_label + question_topic*cat_or_num_question_label,
        data = .)

model_cat_or_num_with_question_type %>% summary()
# num vs cat for aliens is significant

# Clustered Standard Errors and Stargazer Chart
model_basic_cat_or_num$clustered_se <- sqrt(diag(vcovCL(model_basic_cat_or_num, 
                                                        cluster = ~response_id)))

model_cat_or_num_with_treatment$clustered_se <- sqrt(diag(vcovCL(model_cat_or_num_with_treatment, 
                                                        cluster = ~response_id)))

model_cat_or_num_with_question_type$clustered_se <- sqrt(diag(vcovCL(model_cat_or_num_with_question_type, 
                                                        cluster = ~response_id)))

stargazer(model_basic_cat_or_num,
          # model_cat_or_num_with_treatment, 
          model_cat_or_num_with_question_type,
          type="text", 
          se = list(model_basic_cat_or_num$clustered_se,
                    # model_cat_or_num_with_treatment$clustered_se,
                    model_cat_or_num_with_question_type$clustered_se),
          omit = "response_id", align = TRUE)
# ,
#           add.lines = list(c("Individual Fixed Effects", "No", "Yes")))
...
# Clustered Standard Errors and Stargazer Chart
model_basic_change_in_belief$clustered_se <- sqrt(diag(vcovCL(model_basic_change_in_belief, 
                                                        cluster = ~response_id)))

model_basic_change_in_belief_with_prior$clustered_se <- sqrt(diag(vcovCL(model_basic_change_in_belief_with_prior, cluster = ~response_id)))

model_basic_change_in_belief_with_prior_by_topic$clustered_se <- sqrt(diag(vcovCL(model_basic_change_in_belief_with_prior_by_topic, cluster = ~response_id)))
...
stargazer(model_basic_change_in_belief,
          ...
          type = "text",
          dep.var.labels=c("Change In Belief About Topic - Post Treatment (-1 to 1 scale)"),
          column.labels=c("No covariates","With Prior Belief","With Prior Belief & Topic"),
          covariate.labels=c("Treatment","Belief in Topic - Pre", "Recycle Topic", "Social Media Topic", 
                             "Belief in Topic - Pre:Treatment", "Belief in Topic - Pre:Recycle Topic",
                             "Belief in Topic - Pre:Social Media Topic", "Treatment:Recycle Topic", 
                             "Treatment:Social Media Topic", "Treatment:Belief in Topic - Pre:Recycle Topic",
                             "Treatment:Belief in Topic - Pre:Social Media Topic"),
          title="Regression Results with Clustered SE on `Response ID`",
          se = list(model_basic_change_in_belief$clustered_se,
                    model_basic_change_in_belief_with_prior$clustered_se,
                    model_basic_change_in_belief_with_prior_by_topic$clustered_se),
          omit = "response_id", align = TRUE)
```



```{r quiz score on interest level}

#simple regression of quiz score on interest level
dat_knowledge_interest <- d_final_pre_regressions %>% 
  select(response_id, question_topic, treatment_bool,
         quiz_perc_correct, quiz_for_correct, quiz_against_correct,
         alien_interest, recycling_interest, social_media_interest,
         alien_knowledge, recycling_knowledge, social_media_knowledg)

source("recode_interest.R")

#overall interest
model_interest <- dat_interest %>% 
  lm(quiz_perc_correct ~ interest_answer_ohe, data = .)
summary(model_interest)
# significant: 0.03296 (SE: 0.01293) 

model_interest_by_topic <- dat_interest %>% 
  lm(quiz_perc_correct ~ interest_answer_ohe + question_topic, data = .)
summary(model_interest_by_topic)

# alien interest
model_interest_alien <- dat_interest %>% 
  filter(question_topic == "alien") %>% 
  lm(quiz_perc_correct ~ interest_answer_ohe, data = .)

summary(model_interest_alien)
# not significant: 0.02310 (0.01961)

# recycle interest
model_interest_recyc <- dat_interest %>% 
  filter(question_topic == "recycle") %>% 
  lm(quiz_perc_correct ~ interest_answer_ohe, data = .)

summary(model_interest_recyc)
# not significant: 0.02526 (0.02083)

# social interest
model_interest_social <- dat_interest %>% 
  filter(question_topic == "social") %>% 
  lm(quiz_perc_correct ~ interest_answer_ohe, data = .)

summary(model_interest_social)
# not significant: 0.005668 (0.023177)

# Clustered Standard Errors and Stargazer Chart
model_interest$clustered_se <- sqrt(diag(vcovCL(model_interest, 
                                                cluster = ~response_id)))

model_interest_by_topic$clustered_se <- sqrt(diag(vcovCL(model_interest_by_topic, 
                                                cluster = ~response_id)))


stargazer(model_interest,
          model_interest_by_topic,
          type = "text",
          se = list(model_interest$clustered_se,
                    model_interest_by_topic$clustered_se),
          omit = "response_id", align = TRUE)
...
```{r r quiz score on knowledge level}
#simple regression of quiz score on knowledge level
dat_knowledge_interest <- d_final_pre_regressions %>% 
  select(response_id, question_topic, treatment_bool,
         quiz_perc_correct, quiz_for_correct, quiz_against_correct,
         alien_interest, recycling_interest, social_media_interest,
...
source("recode_knowledge.R")

dat_knowledge %>% 
  group_by(question_topic) %>% 
  mutate(topic_count = n()) %>% 
  ungroup() %>% 
  group_by(question_topic, knowledge_answer_ohe) %>% 
  summarize(perc = n()/topic_count) %>% 
  ungroup() %>% 
  unique()

#overall knowledge
model_knowledge <- dat_knowledge %>% 
  lm(quiz_perc_correct ~ knowledge_answer_ohe, data = .)
summary(model_knowledge)
# Not significant: 0.01607 (SE: 0.01346) 

#with topic covariate
model_knowledge_topic <- dat_knowledge %>% 
  lm(quiz_perc_correct ~ knowledge_answer_ohe + question_topic, data = .)

summary(model_knowledge_topic)
# recycle and social are significant

# alien interest
model_knowledge_alien <- dat_knowledge %>% 
  filter(question_topic == "alien") %>% 
  lm(quiz_perc_correct ~ knowledge_answer_ohe, data = .)

summary(model_knowledge_alien)
# Not significant: 0.0004924 (0.0221821)

# recycle interest
model_knowledge_recyc <- dat_knowledge %>% 
  filter(question_topic == "recycle") %>% 
  lm(quiz_perc_correct ~ knowledge_answer_ohe, data = .)

summary(model_knowledge_recyc)
# not significant: 0.005583 (0.022169)

# social interest
model_knowledge_social <- dat_knowledge %>% 
  filter(question_topic == "social") %>%
  lm(quiz_perc_correct ~ knowledge_answer_ohe, data = .)

summary(model_knowledge_social)
# significant: -0.06491 (0.02593)

# Clustered Standard Errors and Stargazer Chart
model_knowledge$clustered_se <- sqrt(diag(vcovCL(model_knowledge, 
                                                cluster = ~response_id)))
model_knowledge_topic$clustered_se <- sqrt(diag(vcovCL(model_knowledge_topic, 
                                                cluster = ~response_id)))

stargazer(model_knowledge,
          model_knowledge_topic,
          # dep.var.labels=c("Model Type"),
          # column.labels=c("all","social media only"),
          title="Regression Results with Clustered SE on `Response ID`",
          type = "text",
          se = list(model_knowledge$clustered_se,model_knowledge_topic$clustered_se),
          omit = "response_id", align = TRUE)
...
dat_time_to_complete_with_quiz_scores  <- d_final_pre_regressions %>% 
  ...
  rename(decile_rank_time_information_page = quantile_rank_time_question_page_submit,
         decile_rank_time_fact_tables = quantile_rank_time_fact_page_submit)

#TODO - QC join
stopifnot(d_final_pre_regressions %>% nrow() == dat_time_to_complete_with_quiz_scores %>% nrow())

# how correlated are the rank time variables
dat_time_to_complete_with_quiz_scores %>% 
  mutate(delta_rank_time = abs(decile_rank_time_information_page - decile_rank_time_fact_tables)) %>% 
  count(delta_rank_time) %>% 
...
model_basic_time_per_topic_narrative <- dat_time_to_complete_with_quiz_scores %>% 
  lm(quiz_perc_correct ~ decile_rank_time_information_page, data = .)


summary(model_basic_time_per_topic_narrative)

dat_time_to_complete_with_quiz_scores %>%
  ggplot(aes(x=decile_rank_time_information_page,y=quiz_perc_correct)) +
  geom_point(alpha=0.25) +
  geom_smooth(method = "lm", 
              mapping = aes(x=decile_rank_time_information_page,y=quiz_perc_correct)) 

dat_time_to_complete_with_quiz_scores %>%
  ggplot(aes(x=factor(decile_rank_time_information_page),y=quiz_perc_correct)) +
  geom_boxplot(outlier.size = 1.5) + 
  geom_smooth(method = "lm", 
              mapping = aes(x=decile_rank_time_information_page,y=quiz_perc_correct))  +
  xlab("Binned Time Spent Reading A Narrative (decile rank)") +
  ylab("Percentage of Quiz Questions Answered Correctly")
...
model_basic_time_per_topic_facts <- dat_time_to_complete_with_quiz_scores %>%
  lm(quiz_perc_correct ~ decile_rank_time_fact_tables, data = .)
...
model_basic_time_per_topic <- dat_time_to_complete_with_quiz_scores %>%
  mutate(decile_rank_total_time = decile_rank_time_information_page + 
       decile_rank_time_fact_tables) %>% 
  lm(quiz_perc_correct ~ decile_rank_total_time, data = .)
  # lm(quiz_perc_correct ~ decile_rank_time_information_page + 
  #      decile_rank_time_fact_tables , data = .)
...
model_basic_time_per_topic_with_treatment <- dat_time_to_complete_with_quiz_scores %>%
  mutate(total_time = decile_rank_time_information_page + 
       decile_rank_time_fact_tables) %>% 
...
dat_time_to_complete_with_quiz_scores %>%
  lm(quiz_perc_correct ~ treatment_bool + decile_rank_time_information_page + 
       treatment_bool*decile_rank_time_information_page,
     data = .) %>% summary()

dat_time_to_complete_with_quiz_scores %>%
  lm(quiz_perc_correct ~ treatment_bool + decile_rank_time_fact_tables + 
       treatment_bool*decile_rank_time_fact_tables,
     data = .) %>% summary()

# Treatment effect was insignificant regardless of time spent

# Part 5 - total time and treatment by topic
model_basic_time_per_topic_with_treatment_by_question <- dat_time_to_complete_with_quiz_scores %>%
  mutate(total_time = decile_rank_time_information_page + 
       decile_rank_time_fact_tables) %>% 
  lm(quiz_perc_correct ~ treatment_bool + total_time + question_topic + 
       treatment_bool*total_time*question_topic,
     data = .)

summary(model_basic_time_per_topic_with_treatment_by_question)


# Clustered SE
model_basic_time_per_topic_narrative$clustered_se <- sqrt(diag(vcovCL(model_basic_time_per_topic_narrative, 
                                                cluster = ~response_id)))
model_basic_time_per_topic_facts$clustered_se <- sqrt(diag(vcovCL(model_basic_time_per_topic_facts, 
                                                cluster = ~response_id)))
model_basic_time_per_topic$clustered_se <- sqrt(diag(vcovCL(model_basic_time_per_topic, 
                                                            cluster = ~response_id)))

stargazer(model_basic_time_per_topic_narrative,
          model_basic_time_per_topic_facts,
          # model_basic_time_per_topic,
          # dep.var.labels=c("Model Type"),
          # column.labels=c("all","social media only"),
          title="Regression Results with Clustered SE on `Response ID`",
          type = "text",
          se = list(model_basic_time_per_topic_narrative$clustered_se,
                    model_basic_time_per_topic_facts$clustered_se#,
                    # model_basic_time_per_topic$clustered_se
                    ),
          omit = "response_id", align = TRUE)
...
# Total 
# 50*1 + 20*10 + 10*20 = $450
# 350 + fees

raffle_winneres <- readRDS(file = "raffle_winners.rds")


raffle_winneres %>% select(twenty) %>% 
  left_join(d_final_pre_regressions %>% 
              select(email_1, referral_person) %>% 
              unique() %>% 
              rename(twenty = email_1), 
            by = "twenty")

raffle_winneres %>% select(five) %>% 
  left_join(d_final_pre_regressions %>% 
              select(email_1, referral_person) %>% 
              unique() %>% 
              rename(five = email_1), 
...
# Replace pharvey with "aba4693@gmail.com" (random selection) as pharvey asked to opt out. 


# # $50 Winner
tier_1_prize <- d_final_pre_regressions[!is.na(email_1) &
                                          email_1 != "",
                                        sample(email_1, 1)]
# tier_1_prize
# # "Andikhosravi@gmail.com"
# 
# # $20 Winner
# tier_2_prizes <- d_final_pre_regressions[!is.na(email_1) 
#                                          & email_1 != "" & 
#                                          !(email_1 %in% tier_1_prize),
#                         sample(email_1, 10)]
# 
# tier_2_prizes
#  # [1] "valkarp@berkeley.edu"       "Kirshtein@gmail.com"        "Tanayndr@gmail.com"         "connormcc@berkeley.edu"    
#  # [5] "Kirshtein@gmail.com"        "giorgia.willits@gmail.com"  "vincegabrielson@gmail.com"  "bethany.vittetoe@gmail.com"
#  # [9] "frankie.pun@berkeley.edu"   "aroeder62@gmail.com" (accidentally sent two to kirshtein... oh well. congrats to them!)
# 
# # $10 Winner
# tier_3_prizes <- d_final_pre_regressions[!is.na(email_1) 
#                                          & email_1 != "" & 
#                                          !(email_1 %in% tier_1_prize) &
#                                          !(email_1 %in% tier_2_prizes) ,
#                         sample(email_1, 20)]
# 
# tier_3_prizes
# #  [1] "Jacksonhaley16@gmail.com"    "awasthianshuman96@gmail.com" "Suna.yeltekin@gmail.com"     "rsingh0206@berkeley.edu"    
# #  [5] "aba4693@gmail.com"        "jjfatta@gmail.com"           "Darthstewie69@gmail.com"     "Petertripp75@gmail.com"     
# #  [9] "robert.turnage@berkeley.edu" "zane.liu@aol.com"            "gailspencer17@gmail.com"     "Arizonakmilotich@gmail.com" 
# # [13] "aysun@berkeley.edu"          "shane.kramer@berkeley.edu"   "ckjcheung@berkeley.edu"     "matt354@gmail.com"
# # [17]  "jpcrom@hotmail.com"        "katyscott@comcast.net"        "trinity.harvey@humboldt.edu"  "zhawtof@gmail.com"
# 
# 
# raffle_winneres <- data.table(fifty = tier_1_prize,
#                               twenty = tier_2_prizes,
#                               five = tier_3_prizes)

# saveRDS(raffle_winneres, file = "raffle_winners.rds")


# Lucas Schroyer, Fisher Kim, Kai Ying, Peter YH Kim

# Thank you so much for taking our survey and congratulations on winning the $__ raffle! Hope you enjoy this Amazon Gift Card!
