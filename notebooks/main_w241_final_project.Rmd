...
quiz_answer_key <- quiz_answer_key_alien %>% 
  rbind(quiz_answer_key_recycle) %>% 
  rbind(quiz_answer_key_social)


quiz_score <- function(col){
  col_name <- names(col)
  ...
  col_correct <- d[[col_name]] == question_answer
  
  d <- d %>% 
    mutate(paste0(col_name,"_correct") = col_correct)
}

d_graded <- quiz_results_raw %>% 
  select(alien_quiz_against_2) %>% 
  quiz_score()
 #apply map next
