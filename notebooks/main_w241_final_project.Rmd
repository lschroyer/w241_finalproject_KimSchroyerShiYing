...
d <- fread('../src/data/W241 Survey StoryTelling Experiment_April 5, 2022_08.54.csv')
...
ggplot(dat, aes(x=quiz_perc_correct, fill=treatment_bool_factor)) + 
...
stopifnot(length(unique(dat_cat_or_num$response_id)) == length(unique(d_4$response_id)))
# None were dropped
...
dat_cat_or_num_2 <- d_4 %>% 
  select(response_id, treatment_question, treatment_bool) %>% 
  unique() 

dat_cat_or_num_2 %>% filter(response_id == "R_0JoNfdq3IWhf0LT") %>% glimpse()

# todo - join the treatment_bool and response id to the cat_or_num df. need to join by question type (e.g. recycle, alien).
# Check that there are still 12 questions per topic and all we added was whether they were in treatment or not per each topic
# 
```
